...
import { at, present } from '@/__tests__/support/expect';
import type { ApiForecast } from '@/types/api';
...
describe('Forecast.fromApi', () => {
  ...
  it('stringifies the coordinates into a position pair', () => {
    expect(present(Forecast.fromApi(apiForecast)).position).toEqual([
      '41.9981',
      '21.4254',
    ]);
  });
  ...
  it('formats each timestamp as UTC DD/MM/YYYY HH:mm', () => {
    const { data } = present(Forecast.fromApi(apiForecast));

    expect(data).toHaveLength(2);
    expect(at(data, 0).time).toBe('15/01/2024 10:30');
    expect(at(data, 1).time).toBe('15/01/2024 11:30');
  });
  ...
  it('is timezone-independent — always formats in UTC', () => {
    ...
    expect(at(present(Forecast.fromApi(apiForecast)).data, 0).time).toBe(
      '15/01/2024 10:30'
    );
  });

  it('preserves the other measurement fields alongside the formatted time', () => {
    const first = at(present(Forecast.fromApi(apiForecast)).data, 0);
  ...
  it('returns empty data when the payload carries none', () => {
    expect(
      present(Forecast.fromApi({ ...apiForecast, data: null })).data
    ).toEqual([]);
  });
  ...
  it('returns an empty position when a coordinate is missing', () => {
    expect(
      present(Forecast.fromApi({ ...apiForecast, latitude: null })).position
    ).toEqual([]);
    expect(
      present(Forecast.fromApi({ ...apiForecast, longitude: null })).position
    ).toEqual([]);
    expect(
      present(Forecast.fromApi({ ...apiForecast, latitude: undefined })).position
    ).toEqual([]);
  });

  it('still maps the data when the coordinates are missing', () => {
    const result = present(Forecast.fromApi({ ...apiForecast, latitude: null }));
