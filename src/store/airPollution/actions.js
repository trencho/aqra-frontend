...
async function setValue({ commit, dispatch, state }, config) {
  ...
  switch (config.input.id) {
    ...
    case 'sensor': {
      const pollutants = await dispatch(
        'getPollutantsBySensorId',
        config.value
      );
    ...
    case 'pollutant':
      if (
        !state.showForAllCitiesInput.value &&
        !state.showForAllSensorsInput.value
      ) {
    ...
    Object.values(state.cities)
      .map((c) =>
        Object.values(c.sensors || {}).map((s) =>
          dispatch('getForecastBySensorId', {
            sensorId: s.sensorId,
            cityName: c.cityName,
          })
        )
...
async function getSensorsForAllCities({ state, dispatch }) {
  ...
  await Promise.all(
    Object.values(state.cities).map((c) =>
      dispatch('getSensorsByCityName', c.cityName)
    )
  );
}
...
async function getForecastForAllCities({ state, dispatch }) {
  ...
  await Promise.all(
    Object.values(state.cities)
      .map((c) =>
        dispatch('getForecastByCoordinatesForCity', {
          position: c.position,
          cityName: c.cityName,
        })
      )
...
async function getForecastByCoordinatesForCity(
  { commit, state },
  { position, cityName }
) {
  const forecast = Object.values(state.cities).find(
...
async function getForecastBySensorId(
  { commit, state },
  { sensorId, cityName }
) {
